...
private:
	...
	std::vector<std::pair<double,std::size_t> > allContributions(std::vector<Point> const& points)const{
		...
		for(Point const& p: xyFront){
			...
			for(Box & b: boxlists[index]){
	         ...
	         for(auto &pp:contributions) 
                 pp.first= round( pp.first * 1.0e9 ) / 1.0e9;      
...
//		std::sort(contributions.begin(),contributions.end());
		std::sort(contributions.begin(),contributions.end(),
			[](pair<double, std::size_t> const& lhs, std::pair<double, std::size_t> const& rhs){
        ...
        std::vector<std::pair<double,std::size_t> > allContributionsv2(std::vector<Point> const& points)const{
...
//		SHARK_RUNTIME_CHECK(points.size() >= k, "There must be at least k points in the set");
		...
		for(std::size_t j = 0; j != 3; ++j){
			auto pos = std::find_if(
				...
				[&](std::pair<double,std::size_t> const& p){
					return p.second== minIndex[j];
				}
			);
			if(pos != result.end())
				result.erase(pos);
...
//		SHARK_RUNTIME_CHECK(points.size() >= k, "There must be at least k points in the set");
		...
		auto result = allContributionsv2(front);
		result.erase(result.begin()+k,result.end());
		return result;
	}
